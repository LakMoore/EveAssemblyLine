...
- Treat every Firestore schema or record-shape change as an explicit one-way migration. Update the appropriate migration utility under `scripts/`, make it idempotent and transactional, and invoke it with the build/deployment command before the new application shape is used. Keep legacy reads, writes, and cleanup confined to that migration utility; application and runtime storage modules must use only the new shape after migration. Report malformed or unmigratable records and never discard them silently.
