...
The pipeline checks the official CCP/EVE build manifest at `https://developers.eveonline.com/static-data/tranquility/latest.jsonl`, which contains the current `buildNumber`. If `sde/raw/version.json` has the same build number, no archive is downloaded. Otherwise it downloads the official JSONL archive from `https://developers.eveonline.com/static-data/eve-online-static-data-latest-jsonl.zip`. The archive URL can be overridden with `SDE_URL`, or an existing archive can be used with `SDE_ARCHIVE=/path/to/sde.zip`. The pipeline extracts JSONL files to `sde/raw/`, converts them to compact JSON arrays in `sde/processed/`, and generates one TypeScript file per table under `src/lib/sde/generated/`, with `src/lib/sde/generated/index.ts` as the barrel export.
