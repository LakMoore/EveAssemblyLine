...
export async function GET(request: Request) {
  try {
    ...
    const startTypeId = Number.isSafeInteger(requestedStartId) && requestedStartId >= 0 ? requestedStartId : 0;
    ...
    return NextResponse.json({
      ...
      previousStartTypeId: startIndex > 0 ? publishedTypes[Math.max(0, startIndex - typeLimit)]._key : null,
      nextStartTypeId: startIndex + typeLimit < publishedTypes.length ? publishedTypes[startIndex + typeLimit]._key : null,
    });
  } catch (error) {
