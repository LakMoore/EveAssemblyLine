...
type AssetLocation = { locationId: number; name: string; locationType: "station" | "structure"; typeId?: number; systemId?: number };
type OptionsRequest = { language?: string; structures?: SuppliedStructure[]; assetLocations?: AssetLocation[] };
...
async function getOptions(request: Request, language: SdeLanguage, suppliedStructures: SuppliedStructure[] = [], assetLocations: AssetLocation[] = []) {
  ...
  const assetStructures = assetLocations
    ...
    .map((location) => {
      ...
      const suppliedStructure = suppliedStructures.find((structure) => structure.id === `structure:${location.locationId}`);
      return {
        ...
        ...(suppliedStructure ? { reprocessingRig: suppliedRigLevel(typeDogma, suppliedStructure.rigs) } : {}),
      } as const;
    })
  ...
  const resolvedStructures = await Promise.all(assetStructures.map(async (location) => {
    ...
    if (cached && cached.expiresAt > now) return { ...location, canReprocess: cached.canReprocess };
    ...
    const canReprocess = structure.services.some((service) => service.name.toLowerCase().includes("reprocess") && service.state === "online");
    structureServiceCache.set(structureId, { canReprocess, expiresAt: now + structureServiceCacheTtlMs });
  ...
  const response = NextResponse.json({ locations, characters, relevantSkillIds, implants: [{ id: "none", name: "No implant", level: 0 }, ...implantNames, ...cloneImplants] });
...
export async function POST(request: Request) {
  try {
    ...
    if (body.assetLocations !== undefined && (!Array.isArray(body.assetLocations) || body.assetLocations.some((location) => !location || !Number.isSafeInteger(location.locationId) || typeof location.name !== "string" || (location.locationType !== "station" && location.locationType !== "structure") || (location.typeId !== undefined && !Number.isSafeInteger(location.typeId)) || (location.systemId !== undefined && !Number.isSafeInteger(location.systemId))))) {
      return NextResponse.json({ error: "assetLocations must be a valid location list." }, { status: 400 });
    }
