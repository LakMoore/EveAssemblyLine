...
import { calculateReprocessingEfficiency, efficiencyForType, reprocessingSkillForType } from "@/lib/planning/reprocessingEfficiency";
...
type OreGroupMaps = { types: Map<number, TypesRecord>; groups: Map<number, GroupsRecord>; typeDogma: Map<number, TypeDogmaRecord>; dogmaAttributes: Map<number, { _key: number; name: string; defaultValue: number }> };
...
function getOreGroupCache(maps: OreGroupMaps): OreGroupCache {
  ...
  oreGroupCache = new Map([...englishGroups].map(([key, entries]) => {
    ...
    return [key, { typeIds: entries.map((entry) => entry.typeId), baseTypeIds: [...byFirstWord.values()].filter((family) => family.length === 1).map(([entry]) => entry.typeId) }];
  }));
...
export async function POST(request: Request) {
  try {
    const body = (await request.json()) as { language?: string; items?: CompressionRequestItem[]; structureTypeId?: number; reprocessingRig?: number; skillLevels?: Record<string, number>; implantLevel?: number; securityStatus?: number; marketId?: number; orderType?: "buy-1-day" | "buy-5-day" | "sell" };
    ...
    const structure = structureTypeId === 0 ? "NPC" : structureTypeId === 35835 ? "Athanor" : structureTypeId === 35836 ? "Tatara" : undefined;
    ...
    if (orderType !== "buy-1-day" && orderType !== "buy-5-day" && orderType !== "sell")
      return NextResponse.json({ error: "Order type must be Buy (1 Day), Buy (5 Day), or Sell." }, { status: 400 });
    const implantLevel = body.implantLevel ?? 0;
    if (structure === undefined)
      return NextResponse.json({ error: "structureTypeId must be 0, Athanor (35835), or Tatara (35836)." }, { status: 400 });
    if (!Number.isSafeInteger(marketId) || !marketHubs.some((market) => market.regionId === marketId))
      return NextResponse.json({ error: "marketId must be a supported market region ID." }, { status: 400 });
    ...
    if (!skillLevels || Array.isArray(skillLevels) || typeof skillLevels !== "object" || Object.keys(skillLevels).length === 0)
      return NextResponse.json({ error: "skillLevels must be a non-empty map of skill IDs to levels." }, { status: 400 });
    if (Object.entries(skillLevels).some(([skillId, level]) => !/^\d+$/.test(skillId) || !Number.isInteger(level) || level < 0 || level > 5))
      return NextResponse.json({ error: "skillLevels must map numeric skill IDs to whole numbers from 0 through 5." }, { status: 400 });
    ...
    const calculatedEfficiency = calculateReprocessingEfficiency({ types, groups, typeDogma, dogmaAttributes }, structure, skillLevels, implantLevel, body.securityStatus, reprocessingRig);
    ...
    const skillTypes = new Map([...oreGroups].map(([key, group]) => [key, group.typeIds.map((typeId) => ({ typeId, name: names.get(typeId) ?? `Type ${typeId}` }))]));
    ...
    const baseCandidates = [...candidateTypeIds].flatMap((compressedTypeId) => {
      ...
      return yields.size > 0 ? [{ typeId: compressedTypeId, name: (names.get(compressedTypeId) ?? `Type ${compressedTypeId}`).replace(/^Compressed /, ""), unitsToReprocess: type.portionSize, efficiency: efficiencyForType(maps, compressedTypeId, calculatedEfficiency, skillLevels), skill, yields }] : [];
    });
    ...
    const candidates = orderType === "sell"
      ? (await Promise.all(baseCandidates.map(async (candidate) => {
        if (![...candidate.yields.keys()].some((typeId) => (body.items ?? []).some((item) => item.typeId === typeId))) return [];
        const baseline = baselineYields.get(groupName(candidate.name));
        const baselineMaterial = baseline ? [...baseline.entries()].find(([, quantity]) => quantity > 0) : undefined;
        const candidateMaterial = baselineMaterial ? candidate.yields.get(baselineMaterial[0]) : undefined;
        const multiplier = baselineMaterial && candidateMaterial ? candidateMaterial / baselineMaterial[1] : 1;
        const orders = await getMarketSellOrders(marketId, candidate.typeId).catch(() => []);
        return orders.map((order) => ({ ...candidate, selectionId: order.orderId, maxRuns: Math.floor(order.volumeRemain / candidate.unitsToReprocess), price: order.price / multiplier, yields: new Map((baseline ?? candidate.yields).entries().map(([typeId, quantity]) => [typeId, quantity * multiplier] as const)) }));
      }))).flat()
      : await Promise.all(baseCandidates.map(async (candidate) => {
        if (![...candidate.yields.keys()].some((typeId) => (body.items ?? []).some((item) => item.typeId === typeId))) return candidate;
        const averageVolume = await getSevenDayAverageVolume(marketId, candidate.typeId).catch(() => undefined);
        return averageVolume === undefined
          ? candidate
          : { ...candidate, maxRuns: Math.floor((averageVolume * (orderType === "buy-5-day" ? 5 : 1)) / candidate.unitsToReprocess) };
      }));
    const efficiencyGroups = [...skillTypes].map(([key, entries]) => {
      ...
      const groupEfficiency = candidate?.efficiency ?? (representativeTypeId === undefined ? calculatedEfficiency.normalOre : efficiencyForType(maps, representativeTypeId, calculatedEfficiency, skillLevels));
      const baseTypeIds = oreGroups.get(key)?.baseTypeIds ?? [];
      return { skillId, skillName, efficiency: groupEfficiency, marketCategories: baseTypeIds.map((typeId) => ({ typeId, name: names.get(typeId) ?? `Type ${typeId}` })) };
    });
    const averageEfficiency = (groups: typeof efficiencyGroups) => groups.length > 0
      ? groups.reduce((total, group) => total + group.efficiency, 0) / groups.length
      : 0;
    const englishSkillName = (group: typeof efficiencyGroups[number]) => group.skillId === undefined ? "" : types.get(group.skillId)?.name.en ?? "";
    const asteroidGroups = efficiencyGroups.filter((group) => !/moon|ice|gas|scrapmetal/i.test(englishSkillName(group)));
    const moonGroups = efficiencyGroups.filter((group) => /moon/i.test(englishSkillName(group)));
    const iceRate = efficiencyGroups.find((group) => /^ice processing$/i.test(englishSkillName(group)))?.efficiency ?? calculatedEfficiency.ice;
    const compressionResult = compressMaterials(body.items, candidates, names);
    const addPackagedVolumes = (items: typeof compressionResult.plan) => items.map((item) => ({
      ...item,
      ...(item.typeId !== undefined && types.get(item.typeId)?.volume !== undefined
        ? { packagedVolume: types.get(item.typeId)?.packagedVolume ?? types.get(item.typeId)?.volume }
        : {}),
    }));
