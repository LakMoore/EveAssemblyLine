...
type StructureListItem = {
  configured: KnownStructure | null;
  asset: EsiStructure | null;
  name: string;
  systemName?: string;
  type?: string;
  size?: StructureSize;
...
export default function LocationsPage() {
  ...
  const assetsByKnownStructureId = new Map(
    displayedEsiStructures
      .map((structure) => [findLocalOverride(structure)?.id, structure] as const)
      .filter((entry): entry is [string, EsiStructure] => entry[0] !== undefined),
  );
  const matchedAssetIds = new Set(
    assetsByKnownStructureId.values().map((structure) => structure.structureId),
  );
  const structureListItems: StructureListItem[] = [
    ...knownStructures.map((structure) => {
      const asset = assetsByKnownStructureId.get(structure.id) ?? null;
      return {
    ...
    ...displayedEsiStructures
      .filter((structure) => !matchedAssetIds.has(structure.structureId))
      .map((asset) => ({
        configured: null,
        asset,
        name: asset.name,
  ...
  const sortedStructureListItems = [...structureListItems].sort((left, right) => {
    if (locationSort === "alphabetical") return left.name.localeCompare(right.name);
    if (locationSort === "totalVolume") {
      return (right.asset?.totalVolume ?? 0) - (left.asset?.totalVolume ?? 0);
    }
    if (locationSort === "totalCount") {
      return (right.asset?.totalCount ?? 0) - (left.asset?.totalCount ?? 0);
    }
      ...
      <section className={styles.panel}>
        <div className={styles.panelHeader}>
          ...
          <div className={styles.locationControls}>
            <div className="flex min-w-0 items-center gap-2">
              ...
              >
                <SelectTrigger id="structures-sort" size="sm">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  <SelectGroup>
            ...
            <span className={styles.panelDescription}>{structureListItems.length} found</span>
          </div>
        </div>
        {structureListItems.length === 0 ? (
          ...
          <div className={styles.knownStructureList}>
            {sortedStructureListItems.map((item) => {
