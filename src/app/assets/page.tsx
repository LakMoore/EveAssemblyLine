...
function isActiveProductionItem(item: StockItem) {
  return (
    item.inBuild === true
...
function emptyLocation(system: SystemOption, structure: StructureOption | null): StockRecord {
  return {
    ...
    structureName: structure?.name ?? "System assets",
    items: [],
  };
}
...
export default function StockPage() {
  ...
  useEffect(() => {
    async function loadPageData(
      ...
      try {
        ...
        const isMarketOrderRecord = (record: StockRecord) => record.source === "marketOrder";
        ...
        await Promise.all([
          ...
          ...(esiResponse.ok
            ? records
                .filter(
                  (record) =>
                    (isEsiRecord(record) || isMarketOrderRecord(record))
                    && !currentKeys.has(locationKey(record)),
                )
  ...
  const assetTypeOptions = uniqueById(
  ...
  async function removeLocation(location: StockRecord) {
    const confirmed = window.confirm(
      `Remove asset location "${location.structureName}" in ${location.systemName}? All assets held there will be deleted.`,
    );
    if (!confirmed) return;
      ...
      <section className={styles.panel}>
        ...
        {locations.length === 0 ? (
          <Empty className={styles.emptyBuildList}>
            <EmptyDescription>
              No asset locations yet. Add a location to begin tracking items.
            </EmptyDescription>
          </Empty>
        ) : visibleLocations.length === 0 ? (
          <Empty className={styles.emptyBuildList}>
            <EmptyDescription>No locations contain the selected asset type.</EmptyDescription>
          </Empty>
        ) : (
    ...
    <Dialog open onOpenChange={(open) => !open && onCancel()}>
      <DialogContent className={styles.importModal} render={<form onSubmit={submit} />}>
        <DialogHeader>
          ...
          <DialogDescription>
            Add a new asset location to track items. You can select a system and optionally a
            specific structure.
          </DialogDescription>
        </DialogHeader>
        <DialogBody>
          <div className="flex flex-col gap-4">
            <div className="flex flex-col gap-2">
              <Label htmlFor="asset-system-search">System</Label>
            ...
            <div className="flex flex-col gap-2">
              ...
              >
                <SelectTrigger aria-label="Asset structure">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  <SelectGroup>
        ...
        {location.source !== "esi" && location.source !== "marketOrder" && (
          <div className={styles.stockCardActions}>
            ...
            <button
              ...
              aria-label={`Remove assets at ${location.structureName}`}
              title="Remove asset location"
              onClick={onRemove}
            >
      ...
      <div className={styles.stockCardTotals}>
        {stockMetrics.length === 0 ? (
          <Empty className={styles.stockMetricsEmpty}>
            <EmptyDescription>No asset metrics available</EmptyDescription>
          </Empty>
        ) : (
    ...
    try {
      ...
      setError(
        resolveError instanceof Error
          ...
          : "Could not resolve the pasted assets.",
      );
    }
    finally {
    ...
    <Dialog open onOpenChange={(open) => !open && onCancel()}>
      <DialogContent className={styles.importModal} render={<form onSubmit={resolveItems} />}>
        <DialogHeader>
          ...
          <DialogDescription>
            Paste the items you want to add or replace in your assets. One item per line. Put the
        ...
        <DialogBody>
          ...
          <Textarea
            ...
            aria-label="Asset items and quantities"
            autoFocus
          />
          {error && (
