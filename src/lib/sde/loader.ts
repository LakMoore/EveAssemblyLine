      ...
      for (const record of records<BlueprintsRecord>("blueprints.json")) {
        ...
        if (manufacturing?.products) {
          for (const product of manufacturing.products) {
            blueprintByBuildProductTypeId.set(
              product.typeID,
              {
          ...
          for (const product of manufacturing.products) {
            productByTypeId.set(
              product.typeID,
              [...(productByTypeId.get(product.typeID) ?? []), product],
            );
          }
        }
        const reaction = record.activities.reaction;
        if (reaction?.materials) {
          reactionMaterialsByBlueprintId.set(record._key, reaction.materials);
        }
        if (reaction?.products) {
          for (const product of reaction.products) {
            ...
            reactionProductByTypeId.set(
              product.typeID,
              [...(reactionProductByTypeId.get(product.typeID) ?? []), product],
            );
          }
        }
        const invention = record.activities.invention;
        for (const product of invention?.products ?? []) {
          blueprintByInventionProductId.set(
            product.typeID,
            [...(blueprintByInventionProductId.get(product.typeID) ?? []), record],
          );
        }
      }
      return {
