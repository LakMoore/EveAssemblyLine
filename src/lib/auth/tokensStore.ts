...
import { CharacterCollectionRecord, CharacterTokenRecord, PendingMergeRecord, SessionRecord, TokenSet } from "./model";
...
export async function getSessions(): Promise<SessionRecord[]> {
	const storage = await initStorage();
	const raw = (await storage.getItem("sessions")) as Array<SessionRecord & { accountId?: string; characterIds?: number[] }> | undefined;
	...
	if (JSON.stringify(raw ?? []) !== JSON.stringify(records)) await storage.setItem("sessions", records);
	return records;
}
...
export async function getCollections(): Promise<CharacterCollectionRecord[]> {
	...
	const legacy = (await storage.getItem("accounts")) as Array<{ accountId: string; characterIds: number[]; createdAt: string; lastSeenAt: string }> | undefined;
	const collections = stored ?? legacy?.map((account) => ({ collectionId: account.accountId, characterIds: account.characterIds, createdAt: account.createdAt, lastSeenAt: account.lastSeenAt })) ?? [];
	const collectionById = new Map(collections.map((collection) => [collection.collectionId, collection]));
	const now = new Date().toISOString();
	for (const character of characters) {
		let collection = character.collectionId ? collectionById.get(character.collectionId) : undefined;
		if (!collection) {
			collection = {
		...
		if (!collection.characterIds.includes(character.characterId)) collection.characterIds.push(character.characterId);
	...
	if (JSON.stringify(stored ?? []) !== JSON.stringify(normalized)) await storage.setItem("collections", normalized);
	return normalized;
}

export async function getCollection(collectionId: string) {
	return (await getCollections()).find((collection) => collection.collectionId === collectionId) ?? null;
}
...
export async function getCollectionForCharacter(characterId: number) {
	...
	return (await getCollections()).find((collection) => collection.characterIds.includes(characterId)) ?? null;
}

export async function createCollection(): Promise<CharacterCollectionRecord> {
	const now = new Date().toISOString();
	const collection = { collectionId: randomUUID(), characterIds: [], createdAt: now, lastSeenAt: now };
	const storage = await initStorage();
...
export async function saveCollection(record: CharacterCollectionRecord) {
	const collections = await getCollections();
	const index = collections.findIndex((collection) => collection.collectionId === record.collectionId);
...
export async function deleteCharacter(characterId: number, collectionId: string) {
	...
	await storage.runTransaction(async (transaction) => {
		...
		if (!character || character.collectionId !== collectionId) throw new Error("Character is not attached to this collection.");

		const storedCollections = (await transaction.getItem("collections")) as CharacterCollectionRecord[] | undefined;
		const legacyAccounts = (await transaction.getItem("accounts")) as Array<{ accountId: string; characterIds: number[]; createdAt: string; lastSeenAt: string }> | undefined;
		const collections = storedCollections ?? legacyAccounts?.map((account) => ({ collectionId: account.accountId, characterIds: account.characterIds, createdAt: account.createdAt, lastSeenAt: account.lastSeenAt })) ?? [];
		for (const collection of collections) {
			collection.characterIds = collection.characterIds.filter((id) => id !== characterId);
		}
		transaction.setItem("characters", characters.filter((record) => record.characterId !== characterId));
...
export async function mergeCollections(targetId: string, sourceId: string) {
	...
	await storage.runTransaction(async (transaction) => {
		const rawCharacters = (await transaction.getItem("characters")) as unknown[] | undefined;
		const characters = (rawCharacters ?? []).map(normalizeCharacter).filter((record): record is CharacterTokenRecord => record !== null);
		const storedCollections = (await transaction.getItem("collections")) as CharacterCollectionRecord[] | undefined;
		const legacyAccounts = (await transaction.getItem("accounts")) as Array<{ accountId: string; characterIds: number[]; createdAt: string; lastSeenAt: string }> | undefined;
		const collections = storedCollections ?? legacyAccounts?.map((account) => ({ collectionId: account.accountId, characterIds: account.characterIds, createdAt: account.createdAt, lastSeenAt: account.lastSeenAt })) ?? [];
		...
		const rawSessions = (await transaction.getItem("sessions")) as Array<SessionRecord & { accountId?: string }> | undefined;
		...
		transaction.setItem("collections", collections.filter((collection) => collection.collectionId !== sourceId));
	});
}
...
export async function upsertCharacter(record: CharacterTokenRecord) {
	...
	await storage.runTransaction(async (transaction) => {
		...
		const records = (raw ?? []).map(normalizeCharacter).filter((existing): existing is CharacterTokenRecord => existing !== null);
...
export async function getPendingMerge(mergeId: string) {
	return (await (await initStorage()).getItem(`pending-merge:${mergeId}`)) as PendingMergeRecord | undefined;
}
